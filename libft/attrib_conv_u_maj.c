...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2015/12/29 15:49:27 by hbeaujou          #+#    #+#             */
/*   Updated: 2015/12/29 16:01:15 by hbeaujou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    ft_cut_init_u_maj(t_cut *t_v, t_var **var, char **str, int count[3])
{
	...
	(*t_v).c = 't';
	(*t_v).neg = 0;
	(*t_v).nbr = ft_nbrlen_ll(var[count[2]]->u_long);
	(*t_v).flag = check_flag(str, count, &(*t_v).nbr, &(*t_v).c);
	(*t_v).check = ft_atoi_ultra(str[count[0]]);
	if (str[count[0]][0] == '.')
		(*t_v).nbr--;
	str[count[0]] = ft_ntoa_base_un(var[count[2]]->u_long, "0123456789");
...
void    ft_cut_flag_u3_maj(t_cut *t_v, char **str, int count[3])
{
	if ((*t_v).flag < -1 && (*t_v).c != '0')
		ft_cut_flag_u3_1maj(t_v, str, count);
	else if ((*t_v).flag > 0 && ((*t_v).c == '0' || (*t_v).c == '.'))
		ft_cut_flag_u3_2maj(t_v, str, count);
