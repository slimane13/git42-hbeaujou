...
void	ft_boucle_f2_big(int flag[26], int g_nbr_crt[26])
{
	...
	{
		calc(backtrack, g_nbr_crt, i2, &g_test_larg2, &g_test_haut2, 2);
		if (g_test_larg2 < g_test_max + 1)
		{
			g_tampon = resitue(g_tmp_calc, (4 * g_nbr_crt[1]), g_taille_max);
...
void	ft_boucle_f3_big(int flag[26], int g_nbr_crt[26])
{
	...
	{
		calc(backtrack, g_nbr_crt, i3, &g_test_larg3, &g_test_haut3, 3);
		if (g_test_larg3 < g_test_max + 1)
		{
			g_tampon = resitue(g_tmp_calc, (4 * g_nbr_crt[2]), g_taille_max);
...
void	ft_boucle_f4_big(int flag[26], int g_nbr_crt[26])
{
	...
	{
		calc(backtrack, g_nbr_crt, i4, &g_test_larg4, &g_test_haut4, 4);
		if (g_test_larg4 < g_test_max + 1)
		{
			g_tampon = resitue(g_tmp_calc, (4 * g_nbr_crt[3]), g_taille_max);
...
void	ft_boucle_f5_big(int flag[26], int g_nbr_crt[26])
{
	...
	ft_strcpy_int(backtrack, g_tmp_calc, g_taille_max * g_taille_max + 1);
	while (i5 < g_iter_max && g_test_haut5 < g_test_max + 1 && g_trouve == 0)
	{
		calc(backtrack, g_nbr_crt, i5, &g_test_larg5, &g_test_haut5, 5);
		if (g_test_larg5 < g_test_max + 1)
		{
			g_tampon = resitue(g_tmp_calc, (4 * g_nbr_crt[4]), g_taille_max);
